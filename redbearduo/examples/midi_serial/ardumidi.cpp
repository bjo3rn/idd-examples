/* This is a modified version of ardumidi to work with RedBearDuo */
/*
...
{
  Serial.write((uint8_t)(command | (channel & 0x0F)));
  Serial.write((uint8_t)(param1 & 0x7F));
  Serial.write((uint8_t)(param2 & 0x7F));
}
...
int midi_message_available() {
  /* 
     ...
     */
  while ((Serial.available() > 0) && ((Serial.peek() & 0b10000000) != 0x80)) {
    Serial.read();
  }
  ...
  byte command = Serial.peek() & 0b11110000;
...
MidiMessage read_midi_message() {
  ...
  message.command  = (midi_status & 0b11110000);
  message.channel  = (midi_status & 0b00001111);
